...
async function applyTemplatedOverlay(sourceImagePath, overlayImagePath, outputPath, template, overlayName) {
    try {
        ...
        // Fixed canvas size - standard 3:2 DSLR resolution
        const CANVAS_WIDTH = 5184;
        const CANVAS_HEIGHT = 3456;
        ...
        // Get metadata from overlay (frame)
...
async function applyTemplatedInstagramOverlay(sourceImagePath, overlayImagePath, outputPath, template) {
    try {
        ...
        console.log(`Processing Instagram photo with dimensions: ${imgMetadata.width}x${imgMetadata.height}`);

        // Calculate the center point
        const centerX = targetWidth / 2;
        const centerY = targetHeight / 2;
// Calculate maximum scale to fit within Instagram dimensions
        const maxScaleWidth = (targetWidth * 0.9) / imgMetadata.width;
        const maxScaleHeight = (targetHeight * 0.9) / imgMetadata.height;
        const maxScale = Math.min(maxScaleWidth, maxScaleHeight);

// Use provided scale but constrain it
        const scale = Math.min(Math.max(template.scale || 0.5, 0.1), maxScale);
        const scaledWidth = Math.round(imgMetadata.width * scale);
        const scaledHeight = Math.round(imgMetadata.height * scale);

        // IMPORTANT: Extract position values with proper fallbacks
        const posX = template.positionX || 0;
        const posY = template.positionY || 0;

        console.log(`Applying Instagram template with scale: ${scale}, position: ${posX},${posY}, rotation: ${template.rotation || 0}`);
        ...
        if (!fs.existsSync(overlayImagePath)) {
            console.error(`Instagram overlay image not found: ${overlayImagePath}`);
            throw new Error('Instagram overlay image not found');
        }
